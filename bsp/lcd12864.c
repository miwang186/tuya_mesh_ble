...
#define  MLCD_SDA_H      Gpio_SetIO(1, 5, TRUE)
#define  MLCD_SDA_L      Gpio_SetIO(1, 5, FALSE)

#define  MLCD_SCL_H      Gpio_SetIO(1, 4, TRUE)
#define  MLCD_SCL_L      Gpio_SetIO(1, 4, FALSE)
...
void MCLD_Write (unsigned char byt)
{
	while(0 == I2C_GetIrq());
	I2C_WriteByte(byt);
	I2C_ClearIrq();
...
void LCD_Set_Pos (unsigned char x, unsigned char y)
{
    ...
    MCLD_Write (0xB0 + y);
    MCLD_Write (0x80);
    MCLD_Write (((x & 0xf0) >> 4) | 0x10);
    MCLD_Write (0x80);
    MCLD_Write ((x & 0x0f) | 0x00);
    MCLD_Write (0x40);
}
/********************************************
    函数功能：以坐标（row，column）为起点显示width(宽)*height(高)的字模
        row：液晶第row行，数值范围0~7
     column：液晶row行的第column个地址，数值范围0~127
       width: 图片宽度0<width<=128
      height: 图片高度，为8的倍数，最大为8倍
reverse_flag: 反白显示标志位，flag=0,常规显示;flag=1,反白显示
         add：字符数据地址
********************************************/
...
void LCD_P8x16Str (unsigned char x, unsigned char y, unsigned char ch[])
{
    ...
    while (ch[j] != '\0')
    {
        ...
        for (i = 0; i < 8; i++)
            MCLD_Write (F8X16[c * 16 + i]);

        MLCD_Stop();
        LCD_Set_Pos (x, y + 1);

        for (i = 0; i < 8; i++)
            MCLD_Write (F8X16[c * 16 + i + 8]);
...
//==============================================================
//函数名：：LCD_NP8x16Str(byte x,byte y,byte *p)
//功能描述：写入一组标准ASCII字符串，反显
//参数：显示的位置（x,y），y为页范围0～7，要显示的字符串
//返回：无
//==============================================================
